...
from pdf_to_table.constant import TextExtractorKey,ArtifactsKey,LayoutParserKey,PdfSaverKey,TableDetectorKey
from pdf_to_table.entity import LayoutParserConfig,PdfSaverConfig,TableDetectorConfig,TextExtractorConfig
import os
...
class Configuration:
    ...
    def get_text_extractor_config(self)->TextExtractorConfig:
        ...
        text_extractor_content=self.config_content.get(TextExtractorKey.TEXT_EXTRATOR_ROOT_KEY)
        root_dir = os.path.join(self.artifact_dir_name,text_extractor_content.get(TextExtractorKey.TEXT_EXTRATOR_ROOT_DIR_KEY))
        csv_dir_name = os.path.join(root_dir,text_extractor_content.get(TextExtractorKey.TEXT_EXTRATOR_CSV_DIR_KEY))
        table_imgs_dir_name = self.get_table_detector_config().table_imgs_dir_name
        excel_file_name = os.path.join(root_dir,text_extractor_content.get(TextExtractorKey.TEXT_EXTRATOR_EXCEL_FILE_NAME_KEY))
        make_dirs([root_dir,csv_dir_name])
        return TextExtractorConfig(root_dir, csv_dir_name, excel_file_name, table_imgs_dir_name)
    ...
    def get_table_detector_config(self)->TableDetectorConfig:
        ...
        table_detector_content = self.config_content.get(TableDetectorKey.TABLE_DETECTOR_ROOT_KEY)
        
        root_dir = os.path.join(self.artifact_dir_name,
        table_detector_content.get(TableDetectorKey.TABLE_DETECTOR_ROOT_DIR_KEY)
        )
        table_imgs_dir_name = os.path.join(
            root_dir,table_detector_content.get(TableDetectorKey.TABLE_DETECTOR_IMAGES_DIR_KEY)
        )
